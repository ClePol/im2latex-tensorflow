...
def Linear(name, inputs, input_dim, output_dim, activation='linear', bias=True, init=None, weightnorm=False, **kwargs):
    ...
    with tf.name_scope(name) as scope:
        ...
        if bias:
            b = tflib.param(name + '.b', numpy.zeros((output_dim,), dtype='float32'))
    ...
    def __call__(self, inputs, state, scope=None):
        gates = tf.nn.sigmoid(
            tflib.ops.Linear(self._name + '.Gates',
                             tf.concat([inputs, state], 1), self._n_in + self._n_hid, 2 * self._n_hid))

        update, reset = tf.split(gates, num_or_size_splits=2, axis=1)
        scaled_state = reset * state

        candidate = tf.tanh(
            tflib.ops.Linear(self._name + '.Candidate',
                             tf.concat([inputs, scaled_state], 1), self._n_in + self._n_hid, self._n_hid))
...
class LSTMCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        c_tm1, h_tm1 = tf.split(state, num_or_size_splits=2, axis=1)
        #gates = tf.layers.dense(tf.concat([inputs, h_tm1], 1), 4 * self._n_hid, activation=tf.nn.sigmoid)
        gates = tflib.ops.Linear(
            self._name + '.Gates',
            tf.concat([inputs, h_tm1], 1),
...
class im2latexAttentionCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, _input, state, scope=None):
        ...
        h_tm1, c_tm1, output_tm1 = tf.split(state, num_or_size_splits=3, axis=1)

        gates = tflib.ops.Linear(
            self._name + '.Gates',
            tf.concat([_input, output_tm1], 1),
            self._n_in + self._n_hid,
            4 * self._n_hid,
            activation='sigmoid')
        ...
        new_state = tf.concat([h_t, c_t, output_t], 1)
...
class FreeRunIm2LatexAttentionCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, _input, state, scope=None):
        h_tm1, c_tm1, output_tm1 = tf.split(state[:, :3 * self._n_hid], num_or_size_splits=3, axis=1)
        ...
        new_state = tf.concat([h_t, c_t, output_t, logits], 1)
...
def FreeRunIm2LatexAttention(name, ctx, input_dim, output_dim, ENC_DIM, DEC_DIM, D, H, W):
    ...
    h0_dec = tf.concat([
        tf.tile(tflib.param(name + '.Decoder.init.h0', np.zeros((1, 3 * DEC_DIM)).astype('float32')), [batch_size, 1]),
        tf.reshape(tf.one_hot(500, output_dim), (batch_size, output_dim)), 1
    ])

    inputs = tf.zeros((batch_size, 160, 100))

    cell = tflib.ops.FreeRunIm2LatexAttentionCell(name + '.AttentionCell', input_dim, output_dim, DEC_DIM, H * W,
                                                  2 * ENC_DIM, V_t)
