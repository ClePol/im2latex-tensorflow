...
def Linear(name, inputs, input_dim, output_dim, activation='linear', bias=True, init=None, weightnorm=False, **kwargs):
    ...
    with tf.name_scope(name) as scope:
        ...
        weight = tf.get_variable(name + '.W', initializer=weight_values)
        ...
        if weightnorm:
            ...
            target_norms = tf.get_variable(name + '.g', initializer=norm_values)
        ...
        if bias:
            ...
            b = tf.get_variable(name + '.b', initializer=numpy.zeros((output_dim,)).astype('float32'),dtype='float32')
...
def im2latexAttention(name, embedding, ctx, input_dim, ENC_DIM, DEC_DIM, D, H, W):
    ...
    h0_dec = tf.tile(
        tf.get_variable(name + '.Decoder.init.h0', initializer=np.zeros((1, 3 * DEC_DIM)).astype('float32')), [batch_size, 1])
